...
c.content_format === 'Series' || c.media_type === 'series'
);

if (seriesContent.length === 0) {
container.innerHTML = '<div class="empty-state"><div class="empty-icon"><i class="fas fa-film"></i></div><h3>No Series Yet</h3><p>This creator hasn\'t uploaded any series content.</p></div>';
return;
}
...
${episodes.map(item => renderUploadCardHTML(item)).join('')}
</div>
</div>
`;
});
