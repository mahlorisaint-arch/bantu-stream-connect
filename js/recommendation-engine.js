...
(function() {
  ...
  class RecommendationEngine {
    constructor(config) {
      ...
      // Freeze the types object to prevent modification
     ...
     */
    async getMultipleRails(railConfigs) {
     ...
     */
    setCollectionContext(collectionId, episodeNumber, playlistId, sortIndex) {
     ...
     */
    async _getNextInCollection(options = {}) {
     ...
     */
    async _getBecauseYouWatchedCollection(options = {}) {
      ...
      try {
        ...
        if (seriesError || !watchedSeries || watchedSeries.length === 0) {
          return [];
        }
     ...
     */
    async _getPopularInSeries(options = {}) {
      const collectionId = options.collectionId || this.currentCollectionId;
      const limit = options.limit || this.defaultLimit;
      const excludeContentId = options.excludeContentId || this.currentContentId;
     ...
     */
    async _getEpisodeRecommendations(options = {}) {
      const collectionId = options.collectionId || this.currentCollectionId;
      const limit = options.limit || this.defaultLimit;
      const excludeContentId = options.excludeContentId || this.currentContentId;
      ...
      try {
        ...
        if (similarError || !similarSeries || similarSeries.length === 0) {
     ...
     */
    async _getQueueContinue(options = {}) {
      if (!this.userId) return [];
      ...
      try {
        ...
        if (!queueContent || queueContent.length === 0) return [];
     ...
     */
    async _getPersonalizedRecommendations(options = {}) {
      if (!this.userId) return [];
     ...
     */
    async _getBecauseYouWatched(options = {}) {
      if (!this.userId) return [];
     ...
     */
    async _getMoreFromCreator(options = {}) {
     ...
     */
    async _getTrendingInInterests(options = {}) {
      if (!this.userId) return [];
      ...
      try {
        ...
        if (!genresData || genresData.length === 0) return [];
     ...
     */
    async _getContinueWatching(options = {}) {
      if (!this.userId) return [];
     ...
     */
    async _getSimilarGenre(options = {}) {
     ...
     */
    async _getFallbackRecommendations(excludeContentId, limit) {
      try {
     ...
     */
    async getRecommendationsForQueue(queueItems, options = {}) {
      ...
      try {
        ...
        if (!queueMeta || queueMeta.length === 0) return [];
        ...
        const filtered = (recommendations || []).filter(rec => rec && !queueIdSet.has(String(rec.id)));
     ...
     */
    async prefetchRecommendations(types) {
      ...
      console.log('📦 Prefetched', results.filter(r => r && r.length > 0).length, 'recommendation types');
